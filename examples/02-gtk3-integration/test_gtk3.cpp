...
std::string generate_demo_html() {
    ...
    sub->new_node("p")->set_node_content(
        "This example tests integration into GTK3 applications with out "
        "the need to utilise the built back end <b>icGTK</b>."
    );
    sub->new_node("p")->set_node_content_f(
        "This HTML has been converted into a PDF using libwk2gtkpdf and "
        "saved as %s",
        std::string(std::filesystem::current_path().string() + "/demo_gtk3.pdf").c_str()
    );
